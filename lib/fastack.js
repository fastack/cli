...
.then(function() {return require('./keys.js')(FASTACK)})
.then(function() {return require('./builder.js')(FASTACK)})
.then(function() {return require('./localServer.js')(FASTACK)})
.then(function() {
	vorpal.show();
})
;
...
vorpal
	.command('deploy', 'deploy to Syncano')
	.action(function(args, callback) {
		var start = new Date();
		FASTACK.localServer.deploy().then(function() {
			console.log(new Date() - start);
			callback();
		});
	});
